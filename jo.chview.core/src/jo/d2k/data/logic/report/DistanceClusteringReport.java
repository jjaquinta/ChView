 ...
 * Window>Preferences>Java>Code Generation.
 */
public class DistanceClusteringReport
{
    ...
    public static String generateReport(ChViewContextBean params)
    {
        ...
        text.append("<html>");
        text.append("<head>");
        text.append("<title>Distance Clustering</title>");
        text.append("</head>");
        text.append("<body>");
        text.append("<h1>Distance Clustering Report</h1>");
        ...
        for (int i = 0; i < starList.size() - 1; i++)
        {
            ...
            for (int j = i + 1; j < starList.size(); j++)
            {
                ...
                double g = getGradient(star1, star2);
                if (min < 0)
                    min = g;
                else
                    min = Math.min(min, g);
        ...
        text.append("<table>");
        text.append("<tr><th>Minimum Distance</th><th>Maximum Distance</th></tr>");
        text.append("<tr><td align=\"right\">"+UtilLogic.format(min)+"</td><td align=\"right\">"+UtilLogic.format(max)+"</td></tr>");
        text.append("</body>");
        text.append("</table>");
        text.append("<table>");
        text.append("<tr><th>Distance</th><th>Number of Clusters</th><th>Average Cluster Size</th></tr>");
        ...
        for (double g : vals)
        {
            ...
            text.append("<tr>");
            text.append("<td align=\"right\">"+UtilLogic.format(g)+"</td>");
    ...
    {
        double overallDelta = max - min;
        double threshold = overallDelta/1000;
    ...
    private static double getGradient(StarBean star1, StarBean star2)
    {
        ...
        for (int i = 0; i < stars.size() - 1; i++)
        {
            ...
            for (int j = i + 1; j < stars.size(); j++)
            {
                ...
                if ((iCluster != null) && (iCluster == jCluster))
                    continue;
                double dist = getGradient(star1, star2);
                if (dist < max)
                {
