...
public class CHVImporter implements IImporter
{
    /* (non-Javadoc)
    ...
    {
        return new String[] { "ChView CHV File", "*.chv" };
    }
    ...
    public void importFile(File f, IImportCallback cb) throws IOException
    {
        ...
        cb.importStart(numStars);
        for (int i = 0; i < numStars && !cb.isCanceled(); i++)
            importStarSystem(is, rnd, mdMap, style, cb);
        if (!cb.isCanceled())
        {
            String postamble = readString(is);
            MetadataBean md = new MetadataBean();
            md.setDomain("db.info");
            md.setIndex(-1);
            md.setKey("notes");
            md.setStringValue(postamble);
            cb.importMetadata(md);
        }
        is.close();
        cb.importDone();
    }

    private void importStarSystem(InputStream is, Random rnd, Map<String,String> metadataMap, Map<String,Object> style, IImportCallback cb) throws IOException
    {
        byte child = 0;
        int numChildren = 0;
    ...
    private StarBean importStar(InputStream is, StarBean parent, Random rnd, Map<String,String> metadataMap, Map<String,Object> style) throws IOException
    {
