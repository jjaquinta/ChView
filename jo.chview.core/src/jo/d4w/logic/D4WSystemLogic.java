...
public class D4WSystemLogic
{
    private static WeakCache<StarBean, SunBean> mSystemCache = new WeakCache<StarBean, SunBean>();
    
    public static SunBean generateSystem(StarBean primaryStar)
    {
        SunBean sun = mSystemCache.get(primaryStar);
        if (sun != null)
            return sun;
        List<StarBean> stars = new ArrayList<StarBean>();
        stars.add(primaryStar);
        for (StarBean s : primaryStar.getChildren())
            stars.add(s);
        SunBean primary = generateSystem(primaryStar, stars);
        for (int i = 1; i < stars.size(); i++)
        {
            StarBean secondaryStar = stars.get(i);
            SunBean secondary = generateSystem(secondaryStar, stars);
            double d = StarExtraLogic.distance(primaryStar, secondaryStar)*63241.1; // conver to AU
            secondary.setA(d);
            secondary.setE(0);
            secondary.setAxialTilt(0);
            dropIn(primary, secondary);
        }
        mSystemCache.put(primaryStar, primary);
        return primary;
    }
    
    private static void dropIn(BodyBean parent, BodyBean child)
    {
        child.setParent(parent);
        for (int i = 0; i < parent.getChildren().size(); i++)
            if (parent.getChildren().get(i).getA() > child.getA())
            {
    ...
    private static SunBean generateSystem(StarBean star, List<StarBean> _companions)
    {
        List<StarBean> companions = new ArrayList<StarBean>();
        companions.addAll(_companions);
        companions.remove(star);
        ...
        system.getSun().setCompanion(companions.size() > 0 ? companions.get(0) : null);
        ...
        {
            double outerPlanetLimit = Double.MAX_VALUE;
            for (StarBean companion : companions)
            {
                /*
                ...
                double thisLimit = ((0.464 + (-0.380 * mu) + (-0.631 * e)
                        + (0.586 * mu * e) + (0.150 * ConstLogic.pow2(e)) + (-0.198
                        * mu * ConstLogic.pow2(e)))
                        * a);
                outerPlanetLimit = Math.min(outerPlanetLimit, thisLimit);
            }
            system.setOuterPlanetLimit(outerPlanetLimit);
        }
    ...
    {        
        try
        {
            ...
            StarBean primary = D4WStarLogic.getByURI(primaryURI);
