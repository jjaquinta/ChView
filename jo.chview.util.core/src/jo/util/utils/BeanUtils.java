        ...
        try
        {
            ...
            for (int i = 0; i < fromProps.length; i++)
            {
				for (int j = 0; j < toProps.length; j++)
				{
	                ...
	                {
//DebugUtils.trace("From: "+fromClass.getName()+", "+fromProps[i].getName()+", "+fromType.getName()+", "+read.getName());	                	
//DebugUtils.trace("  To: "+toClass.getName()+", "+toProps[j].getName()+", "+toType.getName()+", "+write.getName());	                	
	                    Object[] args = new Object[1];
	                    args[0] = read.invoke(from, new Object[0]);
//DebugUtils.trace(" Val: "+args[0]);                        
            ...
            for (int j = 0; j < toProps.length; j++)
            {
                ...
                if ((value instanceof String) && !toType.getName().equals("java.lang.String"))
                {
                    ...
                    if (type.equals("long"))
                        ...
                        DebugUtils.error("BeanUtils.set, need to convert "+type);
                }
                Method write = toProps[j].getWriteMethod();
                if (write != null)
                {
...
//DebugUtils.trace(" Val: "+args[0]);                        
    ...
    public static void sort(List<Bean> ret, String sortBy)
    {
        if (sortBy != null)
            Collections.sort(ret, new BeanComparator(sortBy));        
    }
        ...
        try
        {
            ...
            for (int i = 0; i < beanProps.length; i++)
            {
                ...
                if ((read != null) && (write != null))
                {
                    ...
                    //DebugUtils.trace("Adding property '"+beanProps[i].getName()+" with read="+read.getName()+", write="+write.getName());
                }
            }
        }
        catch (IntrospectionException e1)
