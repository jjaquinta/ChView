...
public class FieldMorph
{
	...
	{
	    DebugUtils.trace("["+mFromPoints.length+"] "+sx+","+sy+" -> "+ex+","+ey);
	...
	private void triangulatePolygon()
	{
		...
		while (idx.length > 3)
		{
			...
			if (ear == null)
			{
			    DebugUtils.error("Illegally formatted polygon!");
				return;
			}
	...
	private int[] isEar(int i, int[] idx)
	{
		...
		if (i == 0)
			...
			upper = 0;
		DebugUtils.trace("trying "+idx[lower]+", "+idx[i]+", "+idx[upper]+" "
		...
		if (cross > 0)
		{
			DebugUtils.trace("rejecting "+idx[lower]+", "+idx[i]+", "+idx[upper]);
			return null;
		}
		*/
		...
		if (myTri.isColinear())
		{
		    DebugUtils.trace("Rejecting "+idx[lower]+", "+idx[i]+", "+idx[upper]+" = "+myTri.toString());
			return null;
		}
		...
		DebugUtils.trace("Ear = "+idx[lower]+", "+idx[i]+", "+idx[upper]+" = "+myTri.toString());
