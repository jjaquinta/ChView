...
public class CSGLogic 
{
	...
	public static void getInteresectingPoints(Poly3D poly)
	{
		...
		{
			List<Line3D> lines = poly.getLinesForPlanes().get(plane);
			DebugUtils.trace("  plane has "+lines.size()+" lines");
	        ...
	        List<Point3D> points = poly.getPointsForPlanes().get(plane);
	        DebugUtils.trace("  plane has "+points.size()+" raw points");
            cullPoints(points);
            DebugUtils.trace("  plane has "+points.size()+" cooked points");
		}
	}
...
//					if (line.size() <= 2)
//						continue;
//					DebugUtils.trace("Found "+line.size()+" colinear");
...
//					planePoints.add(k2);
//					i = -1;
//					DebugUtils.trace("there are "+planePoints.size()+" left");
//					break;
//				}
			Point3D closest = Points3DLogic.getClosest(new Point3D(), planePoints);
...
//			}
	...
	public static final void main(String[] argv)
	{
		...
		DebugUtils.trace((new Date()).toString());
		DebugUtils.trace(mPoly.getPlanes().size()+" Planes:");
		//for (Plane3D p : mPoly.getPlanes())
		///	DebugUtils.trace("  "+p);
		CSGLogic.getInteresectingLines(mPoly);
		DebugUtils.trace((new Date()).toString());
		DebugUtils.trace(mPoly.getLines().size()+" Lines:");
		//for (Line3D l : mPoly.getLines())
		//	DebugUtils.trace("  "+l);
		CSGLogic.getInteresectingPoints(mPoly);
		DebugUtils.trace((new Date()).toString());
		DebugUtils.trace(mPoly.getPoints().size()+" Points:");
		//for (Point3D p : mPoly.getPoints())
		//	DebugUtils.trace("  "+p);
		CSGLogic.getPolygons(mPoly);
		DebugUtils.trace((new Date()).toString());
		DebugUtils.trace(mPoly.getPolys().size()+" Polygons:");
		for (List<Point3D> poly : mPoly.getPolys())
		{
			StringBuffer trace = new StringBuffer("  "+poly.size()+":");
			for (Point3D p : poly)
				trace.append(" "+p);
			DebugUtils.trace(trace.toString());
		}
		/*
		DebugUtils.trace("Sunflow:");
		int triangles = 0;
		...
		points.addAll(ps);
		DebugUtils.trace("object {");
		DebugUtils.trace("	  shader Glass");
		DebugUtils.trace("	  type mesh");
		DebugUtils.trace("	  name polySurfac");
		DebugUtils.trace("	  "+triangles+" "+points.size());
		for (Point3D p : points)
		{
			Point3D n = p.normal();
			DebugUtils.trace("	v "+p.x*10+" "+p.y*10+" "+p.z*10+"    "+n.x+" "+n.y+" "+n.z+"    0 0");
		}
		for (List<Point3D> poly : mPoly.getPolys())
		{
