 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class TemplateTableViewer<T> implements ISelectionChangedListener, ISelectionProvider, PropertyChangeListener
{
    ...
    public T[] getSelectedItems()
    {
        ...
        {
            Object[] objSel = ((IStructuredSelection)selected).toArray();
            T[] typeSel = (T[])(new Object[objSel.length]);
            for (int i = 0; i < objSel.length; i++)
                typeSel[i] = (T)objSel[i];
            return typeSel;
        }
        else
            return null;
